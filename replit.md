...
```
Changelog:
- June 21, 2025. COMPLETED: Comprehensive Business-Scoped Access Control for Merchants - implemented strict business context filtering for merchants (Role ID 2) across ALL API endpoints. Updated middleware to automatically provide merchants access to all their business IDs when no specific business is selected. Fixed business filtering discrepancy by ensuring merchants see all their associated businesses consistently between API calls and SaaS application. Merchants now can only view, edit, and delete data (staff, clients, appointments, services, accounting, settings, etc.) from businesses they own. System enforces complete multi-tenant data isolation for merchant users across all modules while maintaining Super Admin global access privileges.
