...
```
Changelog:
- June 22, 2025. COMPLETED: Comprehensive Merchant Business-Scoped GET Access Control - implemented strict business-scoped access validation for ALL individual record retrieval (GET by ID) endpoints. Updated 6 critical endpoints (Staff, Clients, Appointments, Services, Accounting Transactions, Barber Plans) to prevent merchants (Role ID: 2) from viewing individual records that don't belong to their authorized businesses. Merchants attempting to access unauthorized records receive 403 error: "Access denied. You can only view [resource type] from businesses you have access to." Super Admin retains unrestricted access to all records. System performs fresh database lookups for merchant business associations and validates record ownership before returning data. Verified functionality: merchant with businesses [35,37] blocked from accessing staff/client records from business 25, while maintaining proper access to their own business records.
- June 22, 2025. COMPLETED: Comprehensive Super Admin Business ID Validation System - implemented mandatory business_id validation for ALL Super Admin create operations across entire API. Updated 8 critical endpoints (Clients, Staff, Services, Accounting Transactions, Payment Gateways, Support Tickets, WhatsApp Instances, Barber Plans) to require explicit business_id in request body or x-selected-business-id header. Super Admin can no longer create resources without specifying target business, ensuring deliberate business context selection. Non-Super Admin users maintain existing business access validation. System prevents accidental cross-business data creation while preserving flexible dual-source business_id validation (body OR header). All create operations now return 400 error when business_id missing: "Business ID is required in request body or x-selected-business-id header".
